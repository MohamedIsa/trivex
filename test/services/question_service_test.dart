...
void main() {
  ...
  test(
    'network error → Result.err(NetworkError) on non-200 with error body',
    () async {
      when(
        mockClient.post(
          any,
  ...
  test(
    'network error → Result.err(NetworkError) with generic message when body is not JSON',
    () async {
      when(
        mockClient.post(
          any,
  ...
  test(
    'parse failure → Result.err(ParseError) on malformed JSON response',
    () async {
      when(
        mockClient.post(
          any,
        ...
        (_) async => http.Response('{"questions": "not-a-list"}', 200),
      );
  ...
  test('GameConfig(count: 5) — request body contains "count": 5', () async {
    ...
    ).thenAnswer((_) async => http.Response(_buildFixture(count: 5), 200));
  ...
  test('request body does NOT contain language field', () async {
    ...
    ).thenAnswer((_) async => http.Response(_buildFixture(count: 3), 200));
  ...
  test('response with language: "ar" → FetchResult.language is "ar"', () async {
    when(
      mockClient.post(
        any,
        headers: anyNamed('headers'),
    ...
    const configAr = GameConfig(topic: 'تاريخ', difficulty: 'easy', count: 3);
  ...
  test('response with language: "en" → FetchResult.language is "en"', () async {
    when(
      mockClient.post(
        any,
        headers: anyNamed('headers'),
  ...
  test('fetchQuestions with non-empty excludeQuestions — '
  ...
  test('fetchQuestions with empty excludeQuestions — '
  ...
  group('offline fallback', () {
    late OfflineQuestionCache offlineCache;
    late Directory tempDir;

    setUp(() async {
      tempDir = Directory.systemTemp.createTempSync('hive_svc_test_');
      Hive.init(tempDir.path);
      if (!Hive.isAdapterRegistered(1)) {
        Hive.registerAdapter(QuestionAdapter());
      }
      await Hive.openBox(OfflineQuestionCache.boxName);
      offlineCache = OfflineQuestionCache();
    });
    ...
    test('fetch fails + cache exists — '
        'returns shuffled cached questions with isOffline true', () async {
      // Pre-populate the offline cache.
      const cacheKey = 'Math-easy-en';
      final cachedQuestions = List.generate(
        10,
        (i) => Question(
          id: 'q${i + 1}',
          question: 'Cached Q${i + 1}',
          options: ['A', 'B', 'C', 'D'],
          correctIndex: 0,
      ...
      await offlineCache.save(cacheKey, cachedQuestions);
      ...
      final result = await svc.fetchQuestions(config);

      expect(result, isA<Ok<FetchResult>>());
      final fetchResult = (result as Ok<FetchResult>).value;
      expect(fetchResult.questions.length, 10);
      expect(fetchResult.isOffline, isTrue);
      expect(fetchResult.language, 'en');

      // Questions should be the same set (possibly reordered).
      final ids = fetchResult.questions.map((q) => q.id).toSet();
      final cachedIds = cachedQuestions.map((q) => q.id).toSet();
      expect(ids, cachedIds);
    });

    test('fetch fails + no cache — '
        'returns Result.err as before', () async {
      // No pre-populated cache — just a fresh empty box.
      when(
        mockClient.post(
          any,
          headers: anyNamed('headers'),
    ...
    test('successful fetch — '
        'saves questions to offline cache', () async {
      when(
        mockClient.post(
          any,
          headers: anyNamed('headers'),
